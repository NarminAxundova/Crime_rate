...
raw %>% skim()
# Data Preprocessing
df.num <- raw %>%select_if(is.numeric) %>%select(ViolentCrimesPerPop,everything())
df.num %>% inspect_na()
num_vars <- df.num%>%select(-ViolentCrimesPerPop)%>%names()
for_vars<-c()
...
features <- df.num %>% select(-ViolentCrimesPerPop) %>% names()
...
df.num <- df.num %>% select(ViolentCrimesPerPop,features)


# Standardize (Normalize) 
df.num %>% glimpse()

df.num.new <- df.num[,-1] %>% scale() %>% as.data.frame()
data <- cbind(df.num.new,df.num[,1]) %>%
